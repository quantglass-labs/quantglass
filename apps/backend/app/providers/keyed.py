...
class FinnhubStocksOHLCVProvider:
    ...
    def get_news(self, symbol: str, limit: int = 20) -> list[dict[str, Any]]:
        ...
        for entry in payload[:limit]:
            published_at = datetime.fromtimestamp(entry.get("datetime", 0), UTC).isoformat()
            items.append(
                {
                    ...
                    or f"Latest provider-backed news for {normalized_symbol}.",
                    "sentiment": "neutral",
                }
            )
        return items
...
class PolygonStocksOHLCVProvider:
    ...
    def get_ohlcv(
        ...
        query = urlencode(
            {"adjusted": "true", "sort": "asc", "limit": 5000, "apiKey": self._api_key}
        )
        payload = _fetch_json(
        ...
        return [
            {
                "open_time_utc": datetime.fromtimestamp(row["t"] / 1000, UTC).isoformat(),
                "open": float(row["o"]),
                "high": float(row["h"]),
                "low": float(row["l"]),
                "close": float(row["c"]),
