...
import { Bot, Brain, Cloud, CloudOff, Cpu, Download, ExternalLink, KeyRound, Plug, Plus, Rocket, Route, Save, Scale, Search, Server, ShieldCheck, Sparkles, Terminal, Trash2, Upload, Wind, Zap } from 'lucide-react';
...
import type { AiModelInfo, AiProviderTestResponse, AiSettings, ApiKeyField, CustomProviderProfile, CustomProviderUpsertRequest, ExtensionRegistryEntry, ExtensionSurfaceEntry, IndicatorRegistryEntry, NotificationTestChannel, ProviderCapability, ProviderRegistryEntry, ProviderSettings, SavedStrategy, ScreenState, StrategyRegistryEntry, Timeframe, TradingMode, ViewMode } from '../types';
...
const providerCredentialRequirements: Record<string, ProviderCredentialRequirement> = {
  ...
  openai_compatible: {
    ...
    setupText: 'Save an OpenAI-compatible bearer key in API Keys or the AI tab when your gateway requires one.',
  },
  bedrock: {
    keyIds: ['aws-access-key-id', 'aws-secret-access-key', 'aws-session-token'],
    setupText: 'Save AWS Access Key ID and AWS Secret Access Key in API Keys. Add AWS Session Token when using temporary credentials.',
  },
  vertex: {
    keyIds: ['vertex-ai-access-token'],
    setupText: 'Save a Vertex AI OAuth access token in API Keys, then use a project/location Vertex endpoint.',
  },
};
...
export function SettingsScreen({
  ...
  const providerRegistryByLabel = useMemo(
    () => new Map(providerRegistry.map((entry) => [entry.label ?? providerLabelById[entry.name] ?? entry.name, entry])),
    [providerRegistry],
  );
  ...
  const [draftCustomProvider, setDraftCustomProvider] = useState<CustomProviderUpsertRequest>(blankCustomProviderDraft);
  ...
  const hasAlpacaCredentials = useMemo(() => {
    ...
    const secretKey = apiKeys.find((field) => field.id === 'alpaca-market-data-secret-key')?.configured;
  ...
  const selectedAiModelInfo = draftAiSettings.model ? aiModelById.get(draftAiSettings.model) ?? null : null;
  ...
  const aiProviderTemplates: AiProviderTemplate[] = [
    { id: 'template', label: 'Deterministic template', category: 'local', provider: 'template', baseUrl: '', apiKeyId: null, tier: 'Built-in', description: 'Offline deterministic narration fallback. No model call is made.', authMode: 'No authentication', adapterStatus: 'native', defaultModel: 'deterministic-template' },
    { id: 'ollama', label: 'Ollama', category: 'local', provider: 'ollama', baseUrl: 'http://127.0.0.1:11434', apiKeyId: null, tier: 'Free · Local', description: 'Run open models locally with zero server cost.', authMode: 'No authentication', adapterStatus: 'native' },
    { id: 'lm_studio', label: 'LM Studio', category: 'local', provider: 'lm_studio', baseUrl: 'http://127.0.0.1:1234/v1', apiKeyId: null, tier: 'Free · Local', description: 'On-device GGUF models through the LM Studio server.', authMode: 'No key by default', adapterStatus: 'compatible' },
    { id: 'vllm', label: 'vLLM', category: 'local', provider: 'vllm', baseUrl: 'http://127.0.0.1:8000/v1', apiKeyId: null, tier: 'Free · Self-host', description: 'High-throughput self-hosted OpenAI-compatible inference.', authMode: 'Optional bearer token', adapterStatus: 'compatible' },
    { id: 'llamacpp', label: 'llama.cpp', category: 'local', provider: 'llama_cpp', baseUrl: 'http://127.0.0.1:8080/v1', apiKeyId: null, tier: 'Free · Local', description: 'Lightweight local inference server when launched with OpenAI-compatible API.', authMode: 'No key by default', adapterStatus: 'compatible' },
    { id: 'anthropic', label: 'Anthropic Claude', category: 'cloud', provider: 'anthropic', baseUrl: 'https://api.anthropic.com/v1', apiKeyId: 'anthropic-api-key', tier: 'Paid · API', description: 'Claude models via the native Messages API.', authMode: 'Anthropic API key', adapterStatus: 'native' },
    { id: 'openai', label: 'OpenAI GPT', category: 'cloud', provider: 'openai', baseUrl: 'https://api.openai.com/v1', apiKeyId: 'openai-api-key', tier: 'Paid · API', description: 'GPT-class models and reasoning models from the OpenAI API.', authMode: 'OpenAI API key', adapterStatus: 'native' },
    { id: 'gemini', label: 'Google Gemini', category: 'cloud', provider: 'google_gemini', baseUrl: 'https://generativelanguage.googleapis.com/v1beta', apiKeyId: 'google-gemini-api-key', tier: 'Free tier · Paid', description: 'Gemini models through native generateContent.', authMode: 'Google Gemini API key', adapterStatus: 'native' },
    { id: 'deepseek', label: 'DeepSeek', category: 'cloud', provider: 'deepseek', baseUrl: 'https://api.deepseek.com/v1', apiKeyId: 'deepseek-api-key', tier: 'Paid · Low cost', description: 'Cost-efficient chat and reasoning models through an OpenAI-compatible API.', authMode: 'DeepSeek API key', adapterStatus: 'compatible' },
    { id: 'mistral', label: 'Mistral AI', category: 'cloud', provider: 'mistral', baseUrl: 'https://api.mistral.ai/v1', apiKeyId: 'mistral-api-key', tier: 'Free tier · Paid', description: 'Mistral and Codestral model families.', authMode: 'Mistral API key', adapterStatus: 'compatible' },
    { id: 'groq', label: 'Groq', category: 'cloud', provider: 'groq', baseUrl: 'https://api.groq.com/openai/v1', apiKeyId: 'groq-api-key', tier: 'Free tier · Fast', description: 'Low-latency hosted open model inference.', authMode: 'Groq API key', adapterStatus: 'compatible' },
    { id: 'openrouter', label: 'OpenRouter', category: 'router', provider: 'openrouter', baseUrl: 'https://openrouter.ai/api/v1', apiKeyId: 'openrouter-api-key', tier: 'Pay as you go', description: 'One API key for many hosted model providers.', authMode: 'OpenRouter API key', adapterStatus: 'compatible' },
    { id: 'together', label: 'Together AI', category: 'router', provider: 'together', baseUrl: 'https://api.together.xyz/v1', apiKeyId: 'together-api-key', tier: 'Pay as you go', description: 'Hosted open-source models at scale.', authMode: 'Together API key', adapterStatus: 'compatible' },
    { id: 'azure', label: 'Azure OpenAI', category: 'enterprise', provider: 'azure_openai', baseUrl: 'https://your-resource.openai.azure.com/openai/deployments/your-deployment', apiKeyId: 'azure-openai-api-key', tier: 'Enterprise', description: 'Azure-hosted OpenAI deployments. Configure the deployment endpoint and API version if needed.', authMode: 'Azure API key', adapterStatus: 'compatible', defaultModel: 'your-deployment' },
    { id: 'bedrock', label: 'Amazon Bedrock', category: 'enterprise', provider: 'bedrock', baseUrl: 'bedrock-runtime.us-east-1.amazonaws.com', apiKeyId: 'aws-access-key-id', tier: 'Enterprise', description: 'AWS-managed foundation models through signed Bedrock Runtime requests.', authMode: 'AWS access key + secret', adapterStatus: 'native' },
    { id: 'vertex', label: 'Google Vertex AI', category: 'enterprise', provider: 'vertex', baseUrl: 'https://us-central1-aiplatform.googleapis.com/v1/projects/your-project/locations/us-central1', apiKeyId: 'vertex-ai-access-token', tier: 'Enterprise', description: 'GCP-hosted Gemini publisher models through Vertex AI REST.', authMode: 'Vertex OAuth token', adapterStatus: 'native' },
    { id: 'custom', label: 'Custom endpoint', category: 'enterprise', provider: 'openai_compatible', baseUrl: 'https://', apiKeyId: 'openai-compatible-api-key', tier: 'Self-defined', description: 'Any OpenAI-compatible model endpoint.', authMode: 'Bearer token', adapterStatus: 'compatible' },
  ];
  const activeAiProviderTemplate = aiProviderTemplates.find((template) => template.id === activeAiProviderId) ?? aiProviderTemplates[0];
  const filteredAiProviderTemplates = aiProviderTemplates.filter((template) => {
    const query = aiProviderSearch.trim().toLowerCase();
    if (!query) return true;
    return `${template.label} ${template.description} ${template.baseUrl}`.toLowerCase().includes(query);
  });
  const aiProviderGroups: Array<{ category: AiProviderCategory; label: string; items: AiProviderTemplate[] }> = [
    { category: 'local', label: 'Local runtimes', items: filteredAiProviderTemplates.filter((template) => template.category === 'local') },
    { category: 'cloud', label: 'Cloud APIs', items: filteredAiProviderTemplates.filter((template) => template.category === 'cloud') },
    { category: 'router', label: 'Model routers', items: filteredAiProviderTemplates.filter((template) => template.category === 'router') },
    { category: 'enterprise', label: 'Enterprise', items: filteredAiProviderTemplates.filter((template) => template.category === 'enterprise') },
  ];
  const aiProviderProfiles: Record<AiSettings['provider'], { label: string; endpoint: string; keyMode: string; modelSource: string }> = {
    template: {
  ...
  useEffect(() => {
    setDraftApiKeys(Object.fromEntries(apiKeys.map((field) => [field.id, field.secret ? '' : field.value])));
  }, [apiKeys]);
  ...
  useEffect(() => {
    ...
    const matchingTemplate =
      aiProviderTemplates.find((template) =>
        template.provider === aiSettings.provider &&
        (aiSettings.provider !== 'openai_compatible' || template.baseUrl === aiSettings.baseUrl || template.apiKeyId === aiSettings.apiKeyId)
      ) ?? aiProviderTemplates.find((template) => template.provider === aiSettings.provider) ?? aiProviderTemplates[0];
    setActiveAiProviderId(matchingTemplate.id);
  }, [aiSettings]);

  useEffect(() => {
    if (!aiModelOptions.length) return;
    if (aiModelsFetched && !customAiModelEnabled && !aiModelOptions.includes(draftAiSettings.model)) {
  ...
  function normalizeSavedStrategy(value: unknown, index: number): SavedStrategy | null {
    ...
    const symbolId = typeof candidate.symbolId === 'string'
      ? candidate.symbolId
      : typeof candidate.symbol_id === 'string'
        ...
        : typeof candidate.symbol === 'string'
          ? candidate.symbol
          : '';
    const setupType = typeof candidate.setupType === 'string'
      ? candidate.setupType
    ...
    const id = typeof candidate.id === 'string' && candidate.id.trim()
      ? candidate.id.trim()
      : `${symbolId}-${setupType}-${timeframe}-${Date.now()}-${index}`.toLowerCase().replace(/[^a-z0-9_-]+/g, '-').slice(0, 96);
  ...
  async function exportSavedStrategies() {
    const strategiesToExport = savedStrategies.filter((strategy) => selectedStrategyIds.has(strategy.id));
    ...
    try {
      ...
      setStrategyTransferStatus(`Exported ${strategiesToExport.length} selected strateg${strategiesToExport.length === 1 ? 'y' : 'ies'} to ${path}`);
      return;
    } catch {
    ...
    setStrategyTransferStatus(`Exported ${strategiesToExport.length} selected strateg${strategiesToExport.length === 1 ? 'y' : 'ies'} through browser download.`);
  }
    ...
    try {
      ...
      const candidates = Array.isArray(parsed)
        ...
        : parsed && typeof parsed === 'object' && Array.isArray((parsed as { items?: unknown }).items)
          ...
          : parsed && typeof parsed === 'object' && Array.isArray((parsed as { strategies?: unknown }).strategies)
      ...
      setStrategyTransferStatus(strategies.length ? `Importing ${strategies.length} strateg${strategies.length === 1 ? 'y' : 'ies'}...` : 'No valid strategies found in the selected file.');
  ...
  function goToProviderSetup(providerId?: string) {
    setSearchParams(providerId ? { tab: 'keys', setup: providerSetupId(providerId) } : { tab: 'keys' });
    if (providerId) {
  ...
  function findProviderEntry(label: string, preferredCapability?: ProviderCapabilityName) {
    ...
    const candidates = providerRegistry.filter((entry) => {
      ...
      return entry.name === label || entryLabel === label || matchingProviderIds.includes(entry.name);
    });
    ...
    if (preferredCapability) {
      const capabilityMatch = candidates.find((entry) => entry.capabilities.includes(preferredCapability));
  ...
  function toggleCustomProviderCapability(capability: ProviderCapability) {
    setDraftCustomProvider((current) => {
      ...
      return {
        ...current,
        capabilities: Array.from(capabilities.size ? capabilities : new Set<ProviderCapability>(['ohlcv'])),
      };
    });
  }
  ...
  function aiTemplateIcon(template: AiProviderTemplate) {
    ...
    if (template.id === 'bedrock' || template.id === 'vertex') return <ShieldCheck className="size-4" />;
  ...
  function aiTemplateStatus(template: AiProviderTemplate) {
    if (template.adapterStatus === 'adapter_required') {
      return { label: 'Adapter required', tone: 'text-hold', dot: 'bg-hold', detail: 'This provider needs a native adapter extension before it can fetch models or run narration.' };
    }
    if (template.provider === 'template') {
      return { label: 'Built-in', tone: 'text-buy', dot: 'bg-buy', detail: 'Always available as the deterministic fallback.' };
    }
    if (!template.apiKeyId) {
      return { label: template.category === 'local' ? 'Local endpoint' : 'No key selected', tone: 'text-buy', dot: 'bg-buy', detail: 'No saved API key is required by default.' };
    }
    const field = apiKeysById.get(template.apiKeyId);
    if (field?.configured) {
      return { label: 'Connected', tone: 'text-buy', dot: 'bg-buy', detail: `${field.label} is saved.` };
    }
    return { label: 'Needs setup', tone: 'text-hold', dot: 'bg-hold', detail: `${field?.label ?? template.authMode} is not saved yet.` };
  }
    ...
    if (entry.transport === 'keyed') {
      if (entry.source === 'custom') {
        if (entry.profileConfigured) {
          return {
            ...
            tone: 'text-hold',
            detail: 'Profile credentials are saved, but execution needs a provider extension adapter.',
            missingFields,
          };
        }
        return {
          label: 'needs setup',
          tone: 'text-hold',
          detail: 'Save this custom provider API key in API Keys, then install or enable an adapter extension.',
          missingFields,
        };
      }
      if (entry.configured) {
      ...
      return {
        ...
        detail: missingFields.length
          ...
          : providerCredentialRequirements[entry.name]?.setupText ?? 'Save this provider credential in API Keys.',
        missingFields,
      };
    }
  ...
  function routeSetupItems(labels: string[], preferredCapability: ProviderCapabilityName) {
    ...
    return uniqueLabels.flatMap((label) => {
      ...
      return [{
        ...
        setupText: providerCredentialRequirements[entry.name]?.setupText ?? 'Save this provider credential in API Keys.',
      }];
    });
  }

  function renderProviderMeta(label: string, preferredCapability: ProviderCapabilityName = 'ohlcv') {
    if (!label) {
    ...
    return (
      <div className="space-y-2 text-xs">
        <div className="flex flex-wrap gap-2">
          <span className={`rounded-full border border-border px-2.5 py-1 ${status.tone}`}>{status.label}</span>
        ...
        {entry.transport === 'keyed' && !entry.configured ? (
          <button type="button" className="text-accent underline-offset-4 hover:underline" onClick={() => goToProviderSetup(entry.name)}>
            Set up keys
          </button>
        ) : null}
      </div>
  ...
  const cryptoRouteLabels = [providerSettings.cryptoPrimary, providerSettings.cryptoSecondary, providerSettings.cryptoFallback].filter(Boolean);
  const stocksRouteLabels = [providerSettings.stocksPrimary, providerSettings.stocksSecondary, providerSettings.stocksFallback].filter(Boolean);
  ...
  const providerSetupRows = [
    ...
    const fields = group.keyIds.map((keyId) => apiKeysById.get(keyId)).filter((field): field is ApiKeyField => Boolean(field));
    ...
    ...customProviders
      ...
      .map((provider) => {
        ...
        const fields = keyIds.map((keyId) => apiKeysById.get(keyId)).filter((field): field is ApiKeyField => Boolean(field));
        const missingFields = fields.filter((field) => !field.configured);
        ...
        return {
          ...
          description: 'Custom provider profile credential. Adapter execution still requires an extension.',
          keyIds,
  ...
  const focusedProviderSetup = providerSetupRows.find((row) => row.id === selectedProviderSetup || row.entry?.name === selectedProviderSetup);
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-4 lg:grid-cols-2">
                        <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Crypto route</p>
                          <p className="mt-3 text-lg font-medium text-ink">{cryptoRouteLabels.map(providerDisplayName).join(' -> ') || 'No route selected'}</p>
                          <p className="mt-2 text-sm text-muted">The app tries the primary exchange first, then secondary/fallback providers only when the previous route cannot return usable market data.</p>
                        </div>
                        <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Stocks route</p>
                          <p className="mt-3 text-lg font-medium text-ink">{stocksRouteLabels.map(providerDisplayName).join(' -> ') || 'No route selected'}</p>
                          <p className="mt-2 text-sm text-muted">Public providers work without keys. Keyed providers are used only after their credentials are saved.</p>
                        </div>
                        <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">AI</p>
                          <p className="mt-3 text-lg font-medium text-ink">{aiProviderProfiles[aiSettings.provider].label}</p>
                          <p className="mt-2 text-sm text-muted">Configure local or API model gateways from the AI tab; deterministic narration remains the fallback.</p>
                        </div>
                        <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Trading</p>
                          <p className="mt-3 text-lg font-medium text-ink">{tradingMode === 'live' && liveTradingConfirmed ? 'Live confirmed' : 'Paper guarded'}</p>
                          <p className="mt-2 text-sm text-muted">Live execution is controlled from Risk & Safety and requires saved Alpaca credentials plus explicit confirmation.</p>
                        </div>
                      </div>
                      <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="mt-2 text-sm text-muted">Only selected keyed routes appear here. Use Advanced for full provider diagnostics.</p>
                          </div>
                          <Button variant="secondary" onClick={() => goToProviderSetup()}>API Keys</Button>
                        </div>
                        {routeSetupRequired.length ? (
                          <div className="mt-4 grid gap-3 lg:grid-cols-2">
                            {routeSetupRequired.map((item) => (
                              <div key={`${item.entry.name}-${item.label}`} className="rounded-2xl border border-border bg-surface/40 p-4">
                                <p className="font-medium text-ink">{item.label}</p>
                                <p className="mt-2 text-sm text-muted">{item.detail}</p>
                                <p className="mt-2 text-xs text-muted">{item.setupText}</p>
                                <Button variant="ghost" className="mt-3" onClick={() => goToProviderSetup(item.entry.name)}>
                                  Set up keys
                                </Button>
                              </div>
                            ))}
                          ...
                          <p className="mt-4 text-sm text-buy">Selected routes are available with the current credential setup.</p>
                        )}
                      </div>
                    </div>
                  ) : (
                    <>
                          ...
                          <div className="mt-4 grid gap-3 text-sm text-muted">
                            <label className="space-y-2">
                              ...
                              <select className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={providerSettings.cryptoPrimary} onChange={(event) => onUpdateProviderSetting('cryptoPrimary', event.target.value)}>
                                ...
                                {ohlcvCustomProviderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                              </select>
                      ...
                      <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Custom provider profiles</p>
                            <p className="mt-2 text-sm text-muted">Add provider identities and credentials for community adapters. Profiles appear in routing, but live market-data execution still requires an extension adapter for that provider.</p>
                          </div>
                          <Button variant="secondary" onClick={resetCustomProviderDraft}>New profile</Button>
                        </div>
                        <div className="mt-4 grid gap-4 lg:grid-cols-2">
                          <div className="rounded-2xl border border-border bg-surface/40 p-4">
                            <p className="font-medium text-ink">{editingCustomProviderId ? 'Edit custom provider' : 'Add custom provider'}</p>
                            <div className="mt-4 grid gap-3 text-sm text-muted">
                              <label className="space-y-2">
                                <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Name</span>
                                <input className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.label} placeholder="OpenRouter market data, Internal feed, etc." onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, label: event.target.value })} />
                              </label>
                              <label className="space-y-2">
                                <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Base URL</span>
                                <input className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.baseUrl} placeholder="https://api.example.com/v1" onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, baseUrl: event.target.value })} />
                              </label>
                              <label className="space-y-2">
                                <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Auth mode</span>
                                <select className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.authType} onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, authType: event.target.value as CustomProviderUpsertRequest['authType'] })}>
                              ...
                              {draftCustomProvider.authType === 'api_key_header' ? (
                                <label className="space-y-2">
                                  <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Header name</span>
                                  <input className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.apiKeyHeader ?? ''} placeholder="X-API-Key" onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, apiKeyHeader: event.target.value })} />
                                </label>
                              ) : null}
                              {draftCustomProvider.authType === 'api_key_query' ? (
                                <label className="space-y-2">
                                  <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Query param</span>
                                  <input className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.apiKeyQueryParam ?? ''} placeholder="apikey" onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, apiKeyQueryParam: event.target.value })} />
                                </label>
                              ) : null}
                              <label className="space-y-2">
                                <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Notes</span>
                                <textarea className="min-h-24 w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none" value={draftCustomProvider.notes ?? ''} placeholder="Adapter package, docs URL, supported symbols, rate-limit notes..." onChange={(event) => setDraftCustomProvider({ ...draftCustomProvider, notes: event.target.value })} />
                              </label>
                              <div className="space-y-2">
                                <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Capabilities</span>
                                <div className="flex flex-wrap gap-2">
                                  {(['ohlcv', 'order_book', 'news', 'trading', 'ai'] as ProviderCapability[]).map((capability) => (
                                    <button key={capability} type="button" className={`rounded-full border border-border px-3 py-1.5 text-xs ${draftCustomProvider.capabilities.includes(capability) ? 'bg-accent/15 text-accent' : 'bg-white/[0.03] text-muted'}`} onClick={() => toggleCustomProviderCapability(capability)}>
                              ...
                              <div className="flex items-center justify-between rounded-2xl border border-border bg-white/[0.03] px-4 py-3">
                                <span>Enabled</span>
                                <button type="button" className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${draftCustomProvider.enabled ? 'bg-buy/12 text-buy' : 'bg-white/8 text-muted'}`} onClick={() => setDraftCustomProvider({ ...draftCustomProvider, enabled: !draftCustomProvider.enabled })}>
                                  {draftCustomProvider.enabled ? 'On' : 'Off'}
                                </button>
                              </div>
                              <div className="flex flex-wrap gap-3">
                                <Button disabled={!draftCustomProvider.label.trim()} onClick={saveCustomProviderDraft}>{editingCustomProviderId ? 'Save profile' : 'Add provider'}</Button>
                                <Button variant="secondary" onClick={resetCustomProviderDraft}>Reset</Button>
                              </div>
                            </div>
                          </div>
                          <div className="space-y-3">
                            {customProviders.length ? customProviders.map((provider) => (
                              <div key={provider.id} className="rounded-2xl border border-border bg-surface/40 p-4">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <p className="font-medium text-ink">{provider.label}</p>
                                    <p className="mt-1 text-xs text-muted">{provider.id}</p>
                                  </div>
                                  <span className={`rounded-full border border-border px-2.5 py-1 text-xs ${provider.enabled ? 'text-buy' : 'text-muted'}`}>{provider.enabled ? 'enabled' : 'disabled'}</span>
                                </div>
                                <p className="mt-3 text-xs text-muted">Capabilities: {provider.capabilities.join(', ')}</p>
                                <p className="mt-2 text-xs text-muted">Auth: {provider.authType}{provider.apiKeyId ? ` / key id ${provider.apiKeyId}` : ''}</p>
                                <p className="mt-2 text-xs text-muted">Adapter: profile saved, extension adapter required for market-data execution.</p>
                                {provider.notes ? <p className="mt-2 text-xs text-muted">{provider.notes}</p> : null}
                                <div className="mt-4 flex flex-wrap gap-2">
                                  <Button variant="secondary" onClick={() => editCustomProvider(provider)}>Edit</Button>
                                  {provider.apiKeyId ? <Button variant="ghost" onClick={() => goToProviderSetup(provider.id)}>API key</Button> : null}
                                  <Button variant="danger" onClick={() => onDeleteCustomProvider(provider.id)}>Delete</Button>
                                </div>
                              </div>
                            )) : (
                              <EmptyState title="No custom providers" description="Add a provider profile to prepare credentials and routing for a community adapter." />
                            )}
                          </div>
                        </div>
                      </div>
                          ...
                          {providerInventory.length ? providerInventory.map((entry) => {
                            ...
                            return (
                              <div key={entry.name} className="rounded-2xl border border-border bg-surface/40 px-4 py-3 text-sm">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <span className="font-medium text-ink">{entry.label ?? providerLabelById[entry.name] ?? entry.name}</span>
                                    <p className="mt-1 text-xs text-muted">Capabilities: {entry.capabilities.join(', ') || 'none'}</p>
                                  </div>
                                ...
                                {entry.transport === 'keyed' && !entry.configured ? (
                                  <button type="button" className="mt-2 text-xs text-accent underline-offset-4 hover:underline" onClick={() => goToProviderSetup(entry.name)}>
                                    Set up keys
                                  </button>
                                ) : null}
                              </div>
              ...
              {tab === 'keys' ? (
                <div className="space-y-4">
                  ...
                  <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Provider setup map</p>
                        <p className="mt-2 text-sm text-muted">These are the same keyed providers shown in Advanced Providers. Save the listed keys below to move a provider from needs setup to configured.</p>
                      </div>
                      <Button variant="secondary" onClick={() => setSearchParams({ tab: 'providers' })}>Advanced Providers</Button>
                    </div>
                    <div className="mt-4 grid gap-3 lg:grid-cols-2">
                      {providerSetupRows.map((row) => {
                        ...
                        return (
                          <div key={row.id} className={`rounded-2xl border p-4 text-sm ${isFocused ? 'border-accent/70 bg-accent/10' : 'border-border bg-surface/40'}`}>
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="font-medium text-ink">{row.label}</p>
                                <p className="mt-1 text-xs text-muted">{row.description}</p>
                              </div>
                              <span className={`rounded-full border border-border px-2.5 py-1 text-xs ${status.tone}`}>{status.label}</span>
                            </div>
                            <p className="mt-3 text-xs text-muted">Registry entry: {registryName}</p>
                            <p className="mt-2 text-xs text-muted">Required keys: {row.fields.map((field) => field.label).join(', ') || row.keyIds.join(', ')}</p>
                            <p className="mt-2 text-xs text-muted">{status.detail}</p>
                            {row.missingFields.length ? (
                              <p className="mt-2 text-xs text-hold">Missing: {row.missingFields.map((field) => field.label).join(', ')}</p>
                            ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-4 rounded-2xl border border-border bg-surface/40 px-4 py-3 metric-text text-sm text-ink">
                              {field.secret ? (field.configured ? 'Configured' : 'Not configured') : (field.value || 'Not configured')}
                            </div>
                            <label className="mt-4 block space-y-2 text-sm text-muted">
                              <span className="block text-xs font-semibold uppercase tracking-[0.18em]">{field.secret && field.configured ? 'New value' : 'Stored value'}</span>
                              <input
                                className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none"
                                type={field.secret ? 'password' : 'text'}
                                autoComplete="off"
                                placeholder={field.secret && field.configured ? 'Leave blank to keep current saved key' : ''}
                                value={draftValue}
                                onChange={(event) =>
                            ...
                            <div className="mt-4 flex flex-wrap items-center gap-3">
                              <Button disabled={!canSaveKey} onClick={() => onSaveApiKey(field.id, draftValue)}>Save key</Button>
                              {field.secret && field.configured ? (
                                <Button variant="danger" onClick={() => onSaveApiKey(field.id, '')}>Clear key</Button>
                              ) : null}
                  ...
                  <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-muted">Default is paper. Live mode is gated behind saved Alpaca credentials, explicit confirmation, and backend live-order safety checks.</p>
                      </div>
                      <ShieldCheck className="size-5 text-accent" />
                    </div>
                    <div className="mt-4 flex flex-wrap gap-3">
                      <Button variant={tradingMode === 'paper' ? 'primary' : 'secondary'} onClick={() => onSetTradingMode('paper')}>paper</Button>
                      <button type="button" title={hasAlpacaCredentials ? 'Enable live broker routing' : 'Save Alpaca key ID and secret before enabling live mode'} disabled={!hasAlpacaCredentials} className={`rounded-2xl border border-border px-4 py-2.5 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 ${tradingMode === 'live' ? 'bg-sell/15 text-sell' : 'bg-white/[0.03] text-muted'}`} onClick={onRequestLiveTrading}>
                        live
                      </button>
                      <Button variant="ghost" onClick={() => setSearchParams({ tab: 'keys' })}>Go to API Keys</Button>
                    </div>
                    <p className="mt-3 text-xs text-muted">
                      Live confirmation: {tradingMode === 'live' && liveTradingConfirmed ? 'enabled for the current backend settings' : 'not enabled'}.
                      {hasAlpacaCredentials ? ' Alpaca credentials are saved.' : ' Alpaca credentials are missing.'}
                    </p>
                  </div>
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                ...
                <div className="space-y-5">
                  <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-muted">Connect local, cloud, router, and enterprise LLMs. Model lists are fetched from the selected provider when possible.</p>
                      </div>
                      <Button variant="secondary" onClick={() => applyAiProviderTemplate(aiProviderTemplates.find((template) => template.id === 'custom') ?? activeAiProviderTemplate, false)}>
                      ...
                      <div className="grid min-h-[34rem] lg:grid-cols-[18rem_minmax(0,1fr)]">
                        <div className="border-b border-border p-3 lg:border-b-0 lg:border-r">
                          ...
                          <div className="mt-3 space-y-4">
                            {aiProviderGroups.map((group) => group.items.length ? (
                              <div key={group.category}>
                                <p className="px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted">{group.label}</p>
                                <div className="mt-2 space-y-1">
                                  {group.items.map((template) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <span className={`flex size-8 shrink-0 items-center justify-center rounded-xl border ${aiTemplateTone(template)}`}>{aiTemplateIcon(template)}</span>
                                        <span className="min-w-0 flex-1 truncate">{template.label}</span>
                                        <span className={`size-2 shrink-0 rounded-full ${status.dot}`} title={status.label} />
                                      </button>
                                    );
                                  })}
                                </div>
                              ...
                              <div className="space-y-4">
                                <div className="flex flex-wrap items-start justify-between gap-4">
                                  <div className="flex min-w-0 items-start gap-3">
                                    <span className={`flex size-12 shrink-0 items-center justify-center rounded-2xl border ${aiTemplateTone(activeAiProviderTemplate)}`}>{aiTemplateIcon(activeAiProviderTemplate)}</span>
                                    <div className="min-w-0">
                                      <div className="flex flex-wrap items-center gap-2">
                                        <p className="font-medium text-ink">{activeAiProviderTemplate.label}</p>
                                        <span className="rounded-full border border-border bg-white/[0.04] px-2.5 py-1 text-xs text-muted">{activeAiProviderTemplate.tier}</span>
                                        <span className="rounded-full border border-border bg-white/[0.04] px-2.5 py-1 text-xs text-muted">{activeAiProviderTemplate.adapterStatus.replace('_', ' ')}</span>
                                      </div>
                                      <p className="mt-1 text-sm text-muted">{activeAiProviderTemplate.description}</p>
                                    </div>
                                  </div>
                                  <span className={`inline-flex items-center gap-2 rounded-full border border-border bg-white/[0.04] px-3 py-1.5 text-xs ${activeStatus.tone}`}>
                                ...
                                <div className="rounded-2xl border border-border bg-white/[0.03] p-4 text-sm text-muted">
                                  <div className="grid gap-3 md:grid-cols-4">
                                    <div>
                                      <p className="text-xs font-semibold uppercase tracking-[0.18em]">Provider</p>
                                      <p className="mt-1 text-ink">{aiProviderProfiles[draftAiSettings.provider].label}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                      ...
                                      <p className={`mt-1 ${aiModelsFetched ? 'text-buy' : 'text-hold'}`}>{aiCatalogStatus}</p>
                                    </div>
                                    <div>
                                      <p className="text-xs font-semibold uppercase tracking-[0.18em]">Source</p>
                                ...
                                <div className="grid gap-4 lg:grid-cols-2">
                                  <label className="space-y-2 text-sm text-muted lg:col-span-2">
                                    <span className="block text-xs font-semibold uppercase tracking-[0.18em]">{activeAiProviderTemplate.category === 'enterprise' ? 'Endpoint / deployment URL' : 'Base URL'}</span>
                                    <input
                                      ...
                                      disabled={draftAiSettings.provider === 'template' || activeAiProviderTemplate.adapterStatus === 'adapter_required'}
                                  ...
                                  <label className="space-y-2 text-sm text-muted">
                                    <span className="block text-xs font-semibold uppercase tracking-[0.18em]">API key slot</span>
                                    <select
                                      className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none"
                                    ...
                                    <span className="block text-xs text-muted">
                                      {selectedAiApiKey ? `${selectedAiApiKey.label}: ${selectedAiApiKey.configured ? 'Configured' : draftApiKeys[selectedAiApiKey.id]?.trim() ? 'Draft key entered' : 'Not configured'}` : 'Local runtimes and public test gateways may not need a key.'}
                                    </span>
                                  </label>
                                  ...
                                  <label className="space-y-2 text-sm text-muted">
                                    <span className="flex items-center justify-between gap-3 text-xs font-semibold uppercase tracking-[0.18em]">
                                      ...
                                      <span className={aiModelsFetched ? 'text-buy' : 'text-hold'}>{aiModelsLoading ? 'Fetching' : aiModelsFetched ? 'Live' : 'Fallback'}</span>
                                    </span>
                                    <select
                                      className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none"
                                      value={
                                        customAiModelEnabled
                                          ...
                                          : draftAiSettings.model
                                            ? (aiModelOptions.includes(draftAiSettings.model) ? draftAiSettings.model : '__custom__')
                                            : '__none__'
                                      }
                                      disabled={aiModelsLoading || activeAiProviderTemplate.adapterStatus === 'adapter_required'}
                                      onChange={(event) => {
                                        ...
                                        setCustomAiModelEnabled(false);
                                        setDraftAiSettings({ ...draftAiSettings, model: event.target.value });
                                      }}
                                    >
                                      <option value="__none__">{aiModelsLoading ? 'Fetching latest models...' : 'Select a fetched model'}</option>
                                      {aiModelOptions.map((model) => {
                                        const item = aiModelById.get(model);
                                        const label = item?.label && item.label !== model ? `${item.label} (${model})` : model;
                                        const runtimeLabel = item?.runtimeStatus ? ` - ${aiRuntimeStatusMeta(item.runtimeStatus).label}` : '';
                                        return <option key={model} value={model}>{aiModelsFetched ? `${label}${runtimeLabel}` : `${label} - fallback`}</option>;
                                      })}
                                    ...
                                    <span className="block text-xs text-muted">{aiModelsLoading ? 'Fetching models...' : aiModelDetail}</span>
                                  </label>
                                    ...
                                    <div className="rounded-2xl border border-border bg-white/[0.03] p-4 lg:col-span-2">
                                      ...
                                      <label className="mt-4 block space-y-2 text-sm text-muted">
                                        ...
                                        <input
                                          ...
                                          type="password"
                                          autoComplete="off"
                                          placeholder={selectedAiApiKey.configured ? 'Leave blank to keep current saved key' : 'Paste API key'}
                                      ...
                                      <div className="mt-4 flex flex-wrap gap-3">
                                        <Button disabled={!draftApiKeys[selectedAiApiKey.id]?.trim()} onClick={() => onSaveApiKey(selectedAiApiKey.id, draftApiKeys[selectedAiApiKey.id] ?? '')}>
                                          Save key
                                        </Button>
                                        {selectedAiApiKey.configured ? (
                                          <Button variant="danger" onClick={() => onSaveApiKey(selectedAiApiKey.id, '')}>Clear key</Button>
                                        ) : null}
                                        <Button variant="secondary" onClick={() => setDraftApiKeys((current) => ({ ...current, [selectedAiApiKey.id]: '' }))}>Reset key field</Button>
                                        <Button variant="ghost" onClick={() => setSearchParams({ tab: 'keys' })}>All API keys</Button>
                                      </div>
                                    </div>
                                  ) : null}
                                  ...
                                  {selectedAiModelInfo ? (
                                    <div className="rounded-2xl border border-border bg-white/[0.03] p-4 text-sm text-muted lg:col-span-2">
                                      <div className="grid gap-3 md:grid-cols-5">
                                        <div>
                                          <p className="text-xs font-semibold uppercase tracking-[0.18em]">Selected model</p>
                                          <p className="mt-1 text-ink">{selectedAiModelInfo.label ?? selectedAiModelInfo.id}</p>
                                        </div>
                                        <div>
                                          <p className="text-xs font-semibold uppercase tracking-[0.18em]">Owner / date</p>
                                          <p className="mt-1 text-ink">{selectedAiModelInfo.ownedBy ?? selectedAiModelInfo.createdAt ?? (selectedAiModelInfo.created ? new Date(selectedAiModelInfo.created * 1000).toISOString().slice(0, 10) : 'Provider catalog')}</p>
                                        </div>
                                        <div>
                                          <p className="text-xs font-semibold uppercase tracking-[0.18em]">Input limit</p>
                                          <p className="mt-1 text-ink">{selectedAiModelInfo.inputTokenLimit ? selectedAiModelInfo.inputTokenLimit.toLocaleString() : 'Provider default'}</p>
                                        </div>
                                        <div>
                                          <p className="text-xs font-semibold uppercase tracking-[0.18em]">Output limit</p>
                                          <p className="mt-1 text-ink">{selectedAiModelInfo.outputTokenLimit ? selectedAiModelInfo.outputTokenLimit.toLocaleString() : 'Provider default'}</p>
                                        </div>
                                        <div>
                                          <p className="text-xs font-semibold uppercase tracking-[0.18em]">Runtime</p>
                                          <span className={`mt-1 inline-flex rounded-full border px-2.5 py-1 text-xs ${aiRuntimeStatusMeta(selectedAiModelInfo.runtimeStatus).tone}`}>
                                            {aiRuntimeStatusMeta(selectedAiModelInfo.runtimeStatus).label}
                                          </span>
                                        </div>
                                      </div>
                                      {selectedAiModelInfo.description ? <p className="mt-3 text-xs text-muted">{selectedAiModelInfo.description}</p> : null}
                                      {selectedAiModelInfo.runtimeDetail ? <p className="mt-2 text-xs text-muted">{selectedAiModelInfo.runtimeDetail}</p> : null}
                                    </div>
                                  ) : null}

                                  <label className="space-y-2 text-sm text-muted">
                                    <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Temperature</span>
                                    <input
                                      className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none"
                                      type="number"
                                      min="0"
                                      ...
                                      onChange={(event) => setDraftAiSettings({ ...draftAiSettings, temperature: Number(event.target.value) })}
                                    />
                                  </label>
                                  <label className="space-y-2 text-sm text-muted">
                                    <span className="block text-xs font-semibold uppercase tracking-[0.18em]">Max tokens</span>
                                    <input
                                      className="w-full rounded-2xl border border-border bg-white/[0.04] px-4 py-3 text-ink outline-none"
                                      type="number"
                                      min="32"
                                      ...
                                      onChange={(event) => setDraftAiSettings({ ...draftAiSettings, maxTokens: Number(event.target.value) })}
                                    />
                                  </label>
                                  <label className="space-y-2 text-sm text-muted">
                                    ...
                                    />
                                    <span className="block text-xs text-muted">Use 60-120 seconds for cold-starting large local models.</span>
                                  </label>
                                </div>

                                {aiProviderTestResult ? (
                                  <div className={`rounded-2xl border p-4 text-sm ${aiProviderTestResult.ok ? 'border-buy/25 bg-buy/10 text-buy' : 'border-hold/25 bg-hold/10 text-hold'}`}>
                                    <div className="flex flex-wrap items-center justify-between gap-3">
                                      <div className="flex flex-wrap items-center gap-2">
                                        <p className="font-medium">{aiProviderTestResult.ok ? 'Provider test passed' : 'Provider test failed'}</p>
                                        <span className={`rounded-full border px-2.5 py-1 text-xs ${aiRuntimeStatusMeta(aiProviderTestResult.runtimeStatus).tone}`}>
                                          {aiRuntimeStatusMeta(aiProviderTestResult.runtimeStatus).label}
                                        </span>
                                      </div>
                                      <span className="rounded-full border border-current/20 px-2.5 py-1 text-xs">{aiProviderTestResult.elapsedMs.toLocaleString()} ms</span>
                                    </div>
                                    <p className="mt-2">{aiProviderTestResult.detail}</p>
                                    {aiProviderTestResult.runtimeDetail ? <p className="mt-2 text-xs opacity-80">{aiProviderTestResult.runtimeDetail}</p> : null}
                                    {aiProviderTestResult.source ? <p className="mt-2 text-xs opacity-80">Source: {aiProviderTestResult.source}</p> : null}
                                    {aiProviderTestResult.sample ? (
                                      <p className="mt-3 rounded-2xl border border-current/20 bg-black/10 p-3 text-ink">{aiProviderTestResult.sample}</p>
                                    ) : null}
                                  </div>
                                ) : null}

                                <div className="flex flex-wrap gap-3">
                                  <Button disabled={activeAiProviderTemplate.adapterStatus === 'adapter_required'} onClick={() => { saveDraftAiKeyIfNeeded(); onUpdateAiSettings(draftAiSettings); }}>
                                  ...
                                  <Button
                                    ...
                                    disabled={aiModelsLoading || aiProviderTestLoading || activeAiProviderTemplate.adapterStatus === 'adapter_required' || (draftAiSettings.provider !== 'template' && !draftAiSettings.model.trim())}
                                  >
                                    <Plug className="size-4" />
                                    {aiProviderTestLoading ? 'Testing provider...' : 'Test / fetch models'}
                                  </Button>
                                  <Button variant="secondary" onClick={() => { setDraftAiSettings(aiSettings); setCustomAiModelEnabled(false); }}>Reset</Button>
                                </div>
                              </div>
                            );
                          })()}
                  ...
                  <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                    ...
                    <div className="mt-4 flex items-center justify-between rounded-2xl border border-border bg-surface/40 px-4 py-3 text-sm text-muted">
                      <span>Narration: {draftAiSettings.provider === 'template' ? 'deterministic template' : draftAiSettings.cloudEnabled ? `${draftAiSettings.provider} (fact-checked)` : 'deterministic template'}</span>
                      <button type="button" disabled={draftAiSettings.provider === 'template'} className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] disabled:cursor-not-allowed disabled:opacity-50 ${draftAiSettings.cloudEnabled ? 'bg-buy/12 text-buy' : 'bg-white/8 text-muted'}`} onClick={() => setDraftAiSettings({ ...draftAiSettings, cloudEnabled: !draftAiSettings.cloudEnabled })}>
              ...
              {tab === 'extensions' ? (
                <div className="space-y-5">
                        ...
                        <div key={extension.id} className="rounded-3xl border border-border bg-white/[0.03] p-4">
                          ...
                          {isLoaderNotice ? (
                            <div className="mt-4 rounded-2xl border border-hold/30 bg-hold/10 px-4 py-3 text-sm text-hold">
                              Extension code loading is off. Restart the backend with <code className="rounded-md bg-black/20 px-1.5 py-0.5 text-xs">npm run backend:dev:extensions</code> to discover trusted local packs and installed entry points.
                            </div>
                          ) : (
                            <div className="mt-4 flex items-center justify-between rounded-2xl border border-border bg-surface/40 px-4 py-3 text-sm text-muted">
                    ...
                    <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-muted">Backtesting uses this registry. Extension strategies appear here and can contribute matching setup presets.</p>
                        </div>
                        <span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted">{extensionStrategies.length}</span>
                      </div>
                      <div className="mt-4 space-y-3">
                          ...
                          <div key={strategy.id} className="rounded-2xl border border-border bg-surface/40 p-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {strategy.executable ? <span className="rounded-full border border-buy/30 bg-buy/10 px-2 py-0.5 text-[11px] text-buy">executable</span> : null}
                                <span className={`rounded-full border border-border px-2 py-0.5 text-[11px] ${strategy.source === 'extension' ? 'text-accent' : 'text-muted'}`}>{strategy.source}</span>
                              </div>
                            </div>
                            ...
                            <p className="mt-1 text-xs text-muted">Markets: {strategy.market_types.join(', ')} / Timeframes: {strategy.timeframes.join(', ')}</p>
                            {strategy.extension_id ? <p className="mt-1 text-xs text-accent">Extension: {strategy.extension_id}</p> : null}
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-muted">Computed indicators feed signals and backtests now. Catalog indicators are documented contribution targets for community packs and future chart overlays.</p>
                        </div>
                        <div className="flex flex-wrap justify-end gap-2">
                          <span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted">{extensionIndicators.length} total</span>
                          <span className="rounded-full border border-buy/30 bg-buy/10 px-2.5 py-1 text-xs text-buy">{computedIndicatorCount} computed</span>
                          <span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted">{extensionIndicators.length - computedIndicatorCount} catalog</span>
                        </div>
                      </div>
                      <div className="mt-4 space-y-4">
                        {extensionIndicatorGroups.map(([category, indicators]) => (
                          <div key={category} className="rounded-2xl border border-border bg-surface/40 p-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">{category.replaceAll('_', ' ')}</p>
                              <span className="rounded-full border border-border px-2 py-0.5 text-[11px] text-muted">{indicators.length}</span>
                            </div>
                            <div className="mt-3 grid gap-3">
                              {indicators.map((indicator) => (
                                <div key={indicator.id} className="rounded-2xl border border-border bg-white/[0.03] p-3">
                                  <div className="flex flex-wrap items-center justify-between gap-2">
                                    <p className="text-sm font-medium text-ink">{indicator.name}</p>
                                    <div className="flex flex-wrap gap-1">
                                      <span className={`rounded-full border px-2 py-0.5 text-[11px] ${indicator.maturity === 'computed' ? 'border-buy/30 bg-buy/10 text-buy' : 'border-border text-muted'}`}>{indicator.maturity ?? 'catalog'}</span>
                                      <span className={`rounded-full border border-border px-2 py-0.5 text-[11px] ${indicator.source === 'extension' ? 'text-accent' : 'text-muted'}`}>{indicator.source}</span>
                                    </div>
                                  </div>
                                  <p className="mt-2 text-xs text-muted">{indicator.description}</p>
                                  <p className="mt-2 text-xs text-muted">Inputs: {indicator.inputs.join(', ') || 'none'} / Outputs: {indicator.outputs.join(', ') || 'none'}</p>
                                  {indicator.families?.length ? <p className="mt-1 text-xs text-muted">Families: {indicator.families.join(', ')}</p> : null}
                                  {indicator.extension_id ? <p className="mt-1 text-xs text-accent">Extension: {indicator.extension_id}</p> : null}
                                </div>
                              ))}
                            </div>
                          </div>
                  ...
                  <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                    ...
                    <p className="mt-2 text-sm text-muted">Backend-declared extension points that contributors can target with providers, execution adapters, notifications, import/export flows, data-quality checks, and UI panels.</p>
                    {extensionSurfaceGroups.length ? (
                      <div className="mt-4 grid gap-4 lg:grid-cols-2">
                        {extensionSurfaceGroups.map(([category, surfaces]) => (
                          <div key={category} className="rounded-2xl border border-border bg-surface/40 p-4">
                            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">{category.replace('_', ' ')}</p>
                            <div className="mt-3 space-y-3">
                              {surfaces.map((surface) => (
                                <div key={surface.id} className="rounded-2xl border border-border bg-white/[0.03] p-3">
                                  <div className="flex items-start justify-between gap-3">
                                    <p className="text-sm font-medium text-ink">{surface.name}</p>
                                    <span className={`rounded-full border border-border px-2 py-0.5 text-[11px] ${surface.maturity === 'available' ? 'text-buy' : 'text-hold'}`}>{surface.maturity}</span>
                                  </div>
                                  <p className="mt-2 text-xs text-muted">{surface.description}</p>
                                  <p className="mt-2 text-xs text-muted">Source: {surface.source}{surface.extension_id ? ` / ${surface.extension_id}` : ''}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <EmptyState title="No contribution surfaces reported" description="The backend did not return extension surface metadata." />
                    )}
                  </div>
                </div>
              ) : null}
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-border bg-white/[0.03] p-4">
                    <div>
                      ...
                      <p className="text-sm text-muted">Persisted backtest strategies can be exported, imported, or removed from backend storage.</p>
                      {savedStrategies.length ? <p className="mt-2 text-xs text-muted">{savedStrategies.filter((strategy) => selectedStrategyIds.has(strategy.id)).length} of {savedStrategies.length} selected for export.</p> : null}
                      {strategyTransferStatus ? <p className="mt-2 text-xs text-accent">{strategyTransferStatus}</p> : null}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <input ref={strategyImportRef} type="file" accept="application/json,.json" className="hidden" onChange={(event) => void importSavedStrategies(event.target.files?.[0] ?? null)} />
                      <Button variant="secondary" onClick={() => strategyImportRef.current?.click()}>
                        <Upload className="size-4" />
                        Import
                      </Button>
                      <Button variant="secondary" disabled={!savedStrategies.length || !savedStrategies.some((strategy) => selectedStrategyIds.has(strategy.id))} onClick={() => void exportSavedStrategies()}>
                  ...
                  {savedStrategies.length ? (
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        <Button variant="ghost" onClick={() => setSelectedStrategyIds(new Set(savedStrategies.map((strategy) => strategy.id)))}>
                          Select all
                        </Button>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <label className="flex min-w-0 flex-1 items-start gap-3">
                              ...
                              <span className="min-w-0">
                                ...
                                <span className="mt-2 block text-sm text-muted">{strategy.symbolId} · {strategy.setupType} · {strategy.timeframe}</span>
                                <span className="mt-2 block text-xs text-muted">Saved {formatDateTime(strategy.savedAt)} UTC</span>
                              </span>
                            </label>
                            <div>
                              <Button variant="danger" title={`Delete ${strategy.name}`} onClick={() => onDeleteSavedStrategy(strategy.id)}>
                    ...
                    <EmptyState title="No strategies saved" description="Use Backtesting → Save strategy to populate this tab or import a QuantGlass strategy JSON file." />
                  )}
                </div>
              ) : null}
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                      ...
                      <button type="button" className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-accent hover:text-ink" onClick={() => void openExternalLink('https://github.com/quantglass-labs/quantglass')}>
                        GitHub repository
                    ...
                    <div className="rounded-3xl border border-border bg-white/[0.03] p-4">
                      ...
                      <div className="mt-3 grid gap-2 text-sm">
                        <button type="button" className="inline-flex items-center gap-2 text-accent hover:text-ink" onClick={() => void openExternalLink('https://github.com/quantglass-labs/quantglass/blob/main/LICENSE')}>AGPL license <ExternalLink className="size-4" /></button>
                        <button type="button" className="inline-flex items-center gap-2 text-accent hover:text-ink" onClick={() => void openExternalLink('https://github.com/quantglass-labs/quantglass/blob/main/COMMERCIAL-LICENSE.md')}>Commercial licensing <ExternalLink className="size-4" /></button>
                        <button type="button" className="inline-flex items-center gap-2 text-accent hover:text-ink" onClick={() => void openExternalLink('https://github.com/quantglass-labs/quantglass/blob/main/DISCLAIMER.md')}>Financial disclaimer <ExternalLink className="size-4" /></button>
                        <button type="button" className="inline-flex items-center gap-2 text-accent hover:text-ink" onClick={() => void openExternalLink('https://github.com/quantglass-labs/quantglass/blob/main/THIRD-PARTY-NOTICES.md')}>Third-party notices <ExternalLink className="size-4" /></button>
                      </div>
                    </div>
                  </div>
