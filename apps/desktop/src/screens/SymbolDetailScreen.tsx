...
const TradingViewCandlestickChart = lazy(async () =>
  import('../components/tvChart').then((module) => ({
    default: module.TradingViewCandlestickChart,
  })),
);
...
function computeIndicatorView(
  indicator: IndicatorRegistryEntry,
  candles: Candle[],
  colorIndex: number,
): ComputedIndicatorView {
...
function resolveCandleSource(
  ...
  const aggregateCandidates = corridorItems
    .map((entry) => ({
      ...
      candles:
        marketCandlesByKey[marketCandleKey(entry.symbol, entry.timeframe as Timeframe)] ?? [],
    }))
    .filter(
      (candidate) =>
        candidate.candles.length > 0 &&
        candidate.sourceMinutes < targetMinutes &&
        targetMinutes % candidate.sourceMinutes === 0,
    )
...
export function SymbolDetailScreen({
  ...
  const signalRecord = symbol
    ? (signals.find((entry) => entry.symbolId === symbol.id) ?? null)
    : null;
  ...
  const defaultCorridorItem =
    symbolCorridorItems.find((entry) => entry.timeframe === signalRecord?.signal.timeframe) ??
    symbolCorridorItems[0] ??
    null;
  ...
  const resistanceLevel = useMemo(
    () => (highs.length ? Math.max(...highs.slice(-24)) : 0),
    [highs],
  );
  ...
  const priceIndicators = useMemo(
    () =>
      selectedIndicators.filter(
        (indicator) => indicator.executable && indicator.placement === 'price',
      ),
    [selectedIndicators],
  );
  ...
  const symbolNews = news.filter(
    (item) => item.symbol === (signalRecord?.signal.symbol ?? symbol?.symbol),
  );
  const currentSignalFreshness = signalRecord ? signalFreshness(signalRecord.signal) : null;
  const readyIndicatorCount = indicatorDefinitions.filter(
    (indicator) => indicatorStatus(indicator) === 'ready',
  ).length;
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-border bg-white/[0.03] p-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-end gap-3">
                      <h2 className="metric-text text-3xl text-ink">
                        {formatCurrency(symbol.lastPrice)}
                      </h2>
                      <p className={symbol.changePercent >= 0 ? 'text-buy' : 'text-sell'}>
                        {symbol.changePercent >= 0 ? '+' : ''}
                        {symbol.changePercent.toFixed(1)}%
                      </p>
                    </div>
                    {liveCorridorItem ? (
                      <p className="text-xs text-muted">
                        {usesBackendCandles
                          ? usesExactTimeframe
                            ? `Live backend corridor candles: ${liveCorridorItem.provider} ${liveCorridorItem.timeframe} · latest ${formatDateTime(liveCorridorItem.latest_open_time_utc)} UTC`
                            : `Backend ${liveTimeframe} corridor candles from ${liveCorridorItem.provider}, aggregated to ${timeframe}. Latest source candle ${formatDateTime(liveCorridorItem.latest_open_time_utc)} UTC.`
                          : `Backend corridor is available for ${liveCorridorItem.timeframe} via ${liveCorridorItem.provider}; no closed ${timeframe} candles are available yet.`}
                      </p>
                    ) : null}
                  </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <p className="mt-2 text-[11px] text-muted">
                                    Outputs: {indicator.outputs.join(', ') || 'none'}
                                  </p>
                                </button>
                              );
                            })}
                      ...
                      priceSeries={priceIndicators.flatMap((indicator) => [
                        {
                          ...
                          name: indicator.definition.name,
                        ...
                        ...(indicator.secondary
                          ? [
                              {
                                id: `${indicator.definition.id}-secondary`,
                                name: `${indicator.definition.name} 2`,
                                values: indicator.secondary,
                                color: '#f0b84b',
                              },
                            ]
                          : []),
                        ...(indicator.tertiary
                          ? [
                              {
                                id: `${indicator.definition.id}-tertiary`,
                                name: `${indicator.definition.name} 3`,
                                values: indicator.tertiary,
                                color: '#f05b78',
                              },
                            ]
                          : []),
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {panelIndicators.map((indicator) => (
                    <Panel
                      key={indicator.definition.id}
                      className={indicator.definition.id === 'atr14' ? 'p-4 md:col-span-3' : 'p-4'}
                    >
                      <div className="mb-3 flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-muted">
                            {categoryLabel(indicator.definition.category)}
                          </p>
                        </div>
                        <span
                      ...
                      {indicator.executable ? (
                        <>
                          {indicator.definition.id === 'macd-histogram' ||
                          indicator.placement === 'volume' ? (
                            <BarChart
                              values={indicator.values
                                .slice(-24)
                                .map((value) => (Math.abs(value) > 10000 ? value / 1000 : value))}
                            />
                          ) : (
                            <TinyLineChart
                              values={indicator.values.slice(-24)}
                              tone={
                                indicator.definition.category === 'momentum' ? 'hold' : 'accent'
                              }
                            />
                          )}
                          <p className="mt-3 text-xs text-muted">
                            Latest{' '}
                            {Number.isFinite(indicator.values.at(-1))
                              ? indicator.values.at(-1)?.toFixed(2)
                              : 'n/a'}
                            {indicator.secondary?.length
                              ? ` / secondary ${indicator.secondary.at(-1)?.toFixed(2)}`
                              : ''}
                          </p>
                        </>
                      ) : (
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="space-y-3 rounded-3xl border border-border bg-white/[0.03] p-4">
                      ...
                      {currentSignalFreshness ? (
                        <div className="pt-1">
                          <span
                            className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] ${freshnessClassName(currentSignalFreshness.tone)}`}
                          >
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <Button
                        variant="secondary"
                        onClick={() =>
                          onRunBacktest(symbol.id, signalRecord.signal.confidence_basis.setup_type)
                        }
                      >
                    ...
                    {symbolNews.map((item) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <span
                            className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] ${item.sentiment === 'positive' ? 'bg-buy/12 text-buy' : item.sentiment === 'negative' ? 'bg-sell/12 text-sell' : 'bg-white/8 text-muted'}`}
                          >
                            {item.sentiment}
                          </span>
                        </div>
